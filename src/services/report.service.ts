...
function buildReportHtml(caseRecord: any, reportType: 'pmr' | 'mlef', qrCodeDataUri?: string, digitalSignature?: string) {
...
export async function generateCaseReportPdf(caseId: string | number | bigint, options: { reportType?: 'pmr' | 'mlef' | 'auto' } = {}) {
  ...
  const rawSignature = `${caseRecord.case_number}-${resolvedReportType}-${timestamp}-${process.env.JWT_SECRET || 'secret'}`;
  ...
  const html = buildReportHtml(caseRecord, resolvedReportType, qrCodeDataUri, digitalSignature);
